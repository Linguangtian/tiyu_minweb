...
class StoreController extends Controller
{
    ...
    public function actionSms()
    {
        ...
        $bond_success=[];
        if ($list->tpl_bond_success) {
            $bond_success = json_decode($list->tpl_bond_success, true);
        }
        $bond_fail=[];
        if ($list->tpl_bond_fail) {
            $bond_fail = json_decode($list->tpl_bond_fail, true);
        }
        ...
        foreach ($bond_success as $index => $value) {
            $bond_success[$index] = str_replace("\"", "&quot;", $value);
        }
        foreach ($reservation as $index => $value) {
            $reservation[$index] = str_replace("\"", "&quot;", $value);
        }
        foreach ($bond_fail as $index => $value) {
            $bond_fail[$index] = str_replace("\"", "&quot;", $value);
        }
