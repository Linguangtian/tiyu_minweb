...
//                break;
        ...
        if ($postage_rules->type == 1) {
            // 按重计费
			$myArr=json_decode(json_encode($goods),true)['attr_list'];
			$nc = count($myArr); 
			for($j=0;$j<$nc;++$j)
			{
				if($myArr[$j]['attr_group_name']=='重量')
				{
					$totalWeight=floatval(substr(json_decode(json_encode($goods),true)['attr_list'][$j]['attr_name'],0,-1))* $num;
					break;
				}
			} 
			if($j==$nc)
    ...
    public static function getExpressPriceMore($store_id, $city_id, $goodsList, $province_id)
    {
        ...
        foreach ($goodsList as $row) {
            if (isset($newGoodsList[$row['freight']])) {
				...
				$myWeight= $newGoodsList[$row['freight']]['weight'];
            } else {
